...
public class PolicyEvalManager extends AbstractResource implements Runnable {
	...
	private void useAutonomicFramework(){
		...
		scManager = new CommTPManager<CommFWTouch>("CVM_SC_MGR", 
				this.getClass().getResource("../../tpm/CVMSelfConfig.xml").getPath(), cr);
		useACF = false;
		//scManager = new CommFWTestpoint(cr);
	}
	
	public void run() {
		if(useACF == true) scManager.manage();
		while(true){
			if(m_callQueue.hasNext()){
				CVM_Debug.getInstance().printDebugMessage("Next command found");
...
*/
	...
	{
		CVM_Debug.getInstance().printDebugMessage("PolicyEvalManager : Executing NextCommand: "+m_call.getCallName());
