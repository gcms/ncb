...
public class SkypeAdapter extends NCBBridgeBase
{
	...
	public void createSession(String sID)
	{
				...
				&& m_hmChatSessionMap.containsKey(sID)))
		{
			...
			m_hmCallListMap.put(sID, null);
			
			//New Way
			m_hmSessionCallIdMap.put(sID, null);
			m_hmSessionChatIdMap.put(sID, null);
			CVM_Debug.getInstance().printDebugMessage("SkypeAdapter: Session created- "+sID+".");
	 ...
	 */
	public void addParticipant(String sID, String participantID) throws PartyNotAddedException, NoSessionException
	{
		...
		if(m_hmSessionIdMap.containsKey(sID))
		{
			...
			{
				tempList.add(participantID);
				CVM_Debug.getInstance().printDebugMessage("SkypeAdapter: Participant Added- "+participantID+". SessionID = "+sID+".");
		...
		else
		{
			...
			tempList.add(participantID);
			m_hmSessionIdMap.put(sID, tempList);
			CVM_Debug.getInstance().printDebugMessage("SkypeAdapter: Participant Added- "+participantID+".");
			
		}
		
		//CVM_Debug.getInstance().printDebugMessage(sID+ ", "+ participantID);
		//this.addParticipantToCall(sID, participantID);
		this.addParticpantToApplication(participantID);
	...
	public void enableMedium(String connectionID, String mediumName) throws PartyNotAddedException, NoSessionException
	{
		...
		if(m_hmSessionIdMap.containsKey(sID))
		{
			...
			for(int i=0; tempList !=null && i<tempList.size(); i++)
			{
				...
				if(mediumName.equals(NCBBridgeCapability.LIVE_AV)
						||mediumName.equals("AUDIO")||mediumName.equals("VIDEO"))
				{
					...
					this.addParticipantToCall(sID, participantID);
					if(mediumName.equals("VIDEO")||mediumName.equals(
							NCBBridgeCapability.LIVE_AV))switchToVideo(sID);
				}
				else if(mediumName.equals(NCBBridgeCapability.TEXT))
				{
					m_MediumsEnabled.add(MEDIUM.TEXT);
					this.addParticipantToChat(sID, participantID);
				}
			}
			CVM_Debug.getInstance().printDebugMessage("SkypeAdapter: Media Enabled- "+mediumName+".");
	...
	public void disableMedium(String connectionID, String mediumName) throws PartyNotFoundException, NoSessionException
	{
		...
		if(m_hmSessionIdMap.containsKey(sID))
		{
			...
			{
				participantID = tempList.get(i);
				if(mediumName.equals(NCBBridgeCapability.LIVE_AV)||mediumName.equals(NCBBridgeCapability.AUDIO))
				{
					try {
			...
			CVM_Debug.getInstance().printDebugMessage("SkypeAdapter: Media Disabled- "+mediumName+".");
...
//		If the call cannot be placed notify.
	 ...
	 */
	public boolean isLoggedIn(String userName) throws LoginException
	{
		try
		{
		...
		{
			// TODO Auto-generated catch block
			CVM_Debug.getInstance().printDebugErrorMessage("IsLoggedin() - Exception.");
	...
	{
		return m_hmCallSessionMap.containsKey(sID)|| m_hmChatSessionMap.containsKey(sID);
	}
	 ...
	 */
	public synchronized void sendSchema(String schema, String participantID) 
	{
		b_SendLock = true;
		CVM_Debug.getInstance().printDebugMessage("Schema Sent: "+schema+" \nTo: "+participantID);
		try
		{
			if(!m_hmStreamUserMap.containsKey(participantID))
			{
				//Add the participant to the application bridge.s
				//setAppConnected(false);
				
				addParticpantToApplication(participantID);
				//while(!isAppConnected());
				//setAppConnected(false);
			}
			//##Change recursion for while.
			if(m_hmStreamUserMap.get(participantID) == null || m_hmStreamUserMap.get(participantID).getFriend().getId().equals(""))
			{
			...
			CVM_Debug.getInstance().printDebugMessage("User ID: "+"'"+m_hmStreamUserMap.get(participantID).getFriend().getId()+"'");
			m_hmStreamUserMap.get(participantID).write(schema);
	...
	private static class AdapterSkypeCallListener implements CallListener
	{
		...
		public void callReceived(Call call) throws SkypeException {
			...
			}catch (ConnectorException e) {System.out.println(e);}
	...
	public boolean hasMediumFailed(String sessID, String medium_type) {
		//return false;
		if(medium_type == null ) return false;
/*		if(count == 0) {
			count--;
    ...
    public void destroySession(String sID) {
				...
				|| m_hmChatSessionMap.containsKey(sID)))
		{
			...
			for(SkypeAdapter.MEDIUM mediumName: temp){
				try {
					disableMedium(sID, mediumName.name());
				} catch (PartyNotFoundException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				} catch (NoSessionException e) {
			...
			try {
				if(m_hmChatSessionMap.get(sID)!=null)
						m_hmChatSessionMap.get(sID).leave();
				m_hmChatSessionMap.remove(sID);
				if(m_hmCallSessionMap.get(sID)!=null) 
						m_hmCallSessionMap.get(sID).finish();
			} catch (SkypeException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			m_hmCallSessionMap.remove(sID);
			m_hmCallListMap.remove(sID);
			
			//New Way
			m_hmSessionCallIdMap.remove(sID);
			m_hmSessionChatIdMap.remove(sID);
			CVM_Debug.getInstance().printDebugMessage("SkypeAdapter: Session destroyed- "+sID+".");
