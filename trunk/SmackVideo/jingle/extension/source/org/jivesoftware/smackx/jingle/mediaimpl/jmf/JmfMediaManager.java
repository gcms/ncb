...
public class JmfMediaManager extends JingleMediaManager {
    ...
    public static final int BIDIRECTIONAL_MODE = 2;
    private JingleMediaSession session;
    
    /**
     * Creates a Media Manager instance
     */
    public JmfMediaManager(String type, JingleTransportManager transportManager) {
        super(transportManager);
       
    	this.type = type.toUpperCase();
    	this.mode = BIDIRECTIONAL_MODE;
    	
    	setupPayloads();
    }
    
    public JmfMediaManager(String type, int mode, JingleTransportManager transportManager) {
        super(transportManager);
     ...
     */
    public JmfMediaManager(String mediaLocator, String type, JingleTransportManager transportManager) {
        super(transportManager);
    	this.type = type.toUpperCase();
    	this.mode = BIDIRECTIONAL_MODE;
    ...
    public JmfMediaManager(String mediaLocator, String type, int mode, JingleTransportManager transportManager) {
        super(transportManager);
    	this.type = type.toUpperCase();
    	this.mode = mode;
    	this.mediaLocator = mediaLocator;
    ...
    public JingleMediaSession createMediaSession(final PayloadType payloadType, final TransportCandidate remote, final TransportCandidate local, final JingleSession jingleSession) {
    	...
    	if(type.equals(AUDIO_MANAGER))
        {
    		session = new AudioMediaSession(payloadType, remote, local, mediaLocator, jingleSession); 
        }
    	else if(type.equals(VIDEO_MANAGER))
    	{
    		session = new VideoMediaSession(payloadType, remote, local, mediaLocator, jingleSession, mode);
    	}
    	else return null;
    ...
    private void setupPayloads() {
    	...
    	{
    		payloads.add(new PayloadType.Audio(0,"rgb"));
    		payloads.add(new PayloadType.Audio(0,"jpeg"));
    		payloads.add(new PayloadType.Audio(0,"mpeg"));
    		payloads.add(new PayloadType.Audio(0,"vivo"));
    		payloads.add(new PayloadType.Audio(0,"mov"));
    		payloads.add(new PayloadType.Audio(0,"avi"));
    	}
    }
    ...
    public JingleMediaSession getChannelSession()
    {
    	return session;
    }
