...
% \includegraphics[width=0.40\textwidth]{./fig/uvm}
 \caption{An execution engine described by a model is used to execute a DSML model}
 \label{fig:uvm}
\end{figure}
...
a Broker layer at runtime. The \emph{Broker Manager} is the main element of the execution environment which is the responsible for controlling the flow of execution. This element is the runtime object that realizes a \textsf{Manager} described in the meta-model.
...
The \emph{Resource Manager} provides an interface for querying and obtaining its managed resources. These resources are returned as instances of \textsf{ManagedResource}, a class that wraps the actual resource along with its metadata.
...
The use of MDE and DSMLs certainly brings benefits to the development and maintenance of complex applications by providing abstractions closer to the application problem domain. Meanwhile, the development of execution engines for DSMLs still requires a lot of effort and is largely based on code-centric approaches.

In this paper we have proposed a model-driven approach for the development and usage of execution engines for DSMLs targeted at the construction of high-level services upon a set of heterogeneous resources. This approach relies on a generic architecture that addresses the issues involved in the execution of models described in these DSMLs and that can be created and changed at runtime.
%- Contributions: 
...
By doing so, we expect to simplify the development of execution engines for DSMLs, extending the benefits of MDE beyond the construction of applications to the construction of their execution engines. Our work leverages the solutions employed in the  communication and microgrid management domains to provide a systematic 

%Therefore, we have identified  common aspects in the processing of these DSMLs and proposed a model-driven approach to employ the solutions identified in the communication and microgrid management domains
