...
Taking that into account, we propose a model-driven approach to the construction of these execution engines that stems from the solutions employed in the development of CVM and MGridVM. Doing so, we look forward a way to assist the construction of these DSML execution engines. Our proposal relies on a generic layered architecture and the use of modeling as a way to specialize these architecture into an execution engine for a given DSML.
...
\begin{figure}
 \centering
 \includegraphics[width=0.40\textwidth]{./fig/uvm}
 \caption{Generic architecture for a DSML execution engine}
 \label{fig:uvm}
\end{figure}
...
The figure ~\ref{} illustrates the flow of control for a Broker layer at runtime. The \emph{Broker Manager} is the main element of the execution environment which is the responsible for controlling the flow of execution. This element is the runtime object that realizes a \textsf{Manager} described in the meta-model.
