...
A Seção~\ref{sec:arquitetura} descreve uma arquitetura genérica para máquinas de execução de modelos que se enquadram na categoria identificada.
A Seção~\ref{sec:sb}, por sua vez, detalha o subconjunto da abordagem proposta que foi desenvolvido neste trabalho.
...
A construção de um metamodelo e uma máquina de execução projetados para uma DSML permite definir completamente a sintaxe e semântica da linguagem. Enquanto o metamodelo descreve a sintaxe e semântica estática da linguagem, sua semântica dinâmica é capturada de forma operacional pela máquina virtual.
...
Essa categoria, que inclui a CVM \cite{cvm} e MGridVM \cite{microgrid}, contempla máquinas virtuais para execução de DSMLs que descrevem um serviço de alto nível a ser realizado a partir de um conjunto heterogêneo de recursos. 
...
No entanto, para que seja vantajoso o emprego de uma linguagem de modelagem para a descrição de uma máquina de execução, esta linguagem deve possuir uma abrangência suficiente para que possa ser utilizada em diferentes domínios de negócio. 
...
Esses operações, que englobam  transformação e negociação de modelos, avaliação de políticas, recuperação de falhas, seleção e configuração de recursos, entre outras, podem ser tratados como um domínio técnico. 
...
Assim,  essa linguagem incorpora uma arquitetura genérica que pode ser especializada para produzir máquinas de execução específicas domínio.
A arquitetura proposta é uma generalização da arquitetura em camadas empregada pela CVM e MGridVM e inclui as seguintes camadas:
...
Ao nos basearmos na arquitetura da CVM e MGridVM, aproveitamos o conhecimento adquirido em relação à separação de responsabilidades necessárias para a realização de serviços no domínio técnico ao qual ambas pertencem, conforme descrito acima. No entanto, apesar de apresentar a mesma estrutura, a arquitetura proposta difere por ser independente do domínio de negócio.

A construção de uma linguagem para modelagem máquinas de execução de modelos, por sua vez, também requer a construção de um metamodelo que forneça abstrações que possam ser utilizadas para descrever máquinas de execução de acordo com a arquitetura proposta. 
...
Devido ao extenso trabalho envolvido na construção desse metamodelo, neste trabalho construímos um metamodelo que engloba apenas os elementos associados à camada de intermediação de serviços da arquitetura proposta.
A construção do metamodelo representa uma etapa em direção à realização da visão proposta pela abordagem, e tem como objetivo investigar sua viabilidade. 
...
Desta forma, o metamodelo e o ambiente de execução construídos posssibilitam que intermediadores de serviços sejam descritos por meio de modelos e utilizados em tempo de execução pela camada de \emph{middleware}.
Os próximos capítulos apresentam, na seguinte ordem: o metamodelo elaborado para a definição de intermediadores de serviços; o ambiente de execução capaz de processar os modelos descritos por meio do metamodelo; e um modelo criado a partir desse metamodelo para descrever uma camada com comportamento equivalente à camada NCB que integra a plataforma CVM.
