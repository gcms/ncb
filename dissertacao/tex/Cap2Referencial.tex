...
\section{Auto-gerenciamento}

para amenizar a coplexidade do gerenciamento...
self-management architectures
policies for self-management

CVM emprega computação autônoma e políticas para seleção de frameworks
...
Políticas podem ser utilizadas para definir regras para se atingir diversos objetivos de gerenciamento como escalabilidade, performance, otimização do uso de recursos, flexibilidade, etc \cite{Sloman}. Políticas são definidas por linguagens de definição de políticas que oferecem meios para especificar regras que podem ser mapeadas para ações de gerenciamento de um sistema.
...
Na implementação da camada NCB da CVM políticas são utilizadas para direcionar a seleção de \emph{frameworks} de comunicação \cite{ncbPolicies}.
