...
\chapter{Estudo de caso: Intermediador de Comunicação em Rede}
\label{cap:instancia}
...
Ao longo das subseções seguintes, as partes que integram o modelo da camada NCB são descritas por meio de diagramas UML e, quando necessário, por uma notação para descrição de modelos de forma textual e compreensível.
...
Os eventos para os quais não foram definidos tratadores são automaticamente encaminhados para a camada superior, e representam eventos que a camada NCB não é capaz de tratar.
...
Mesmo assim, melhorias significativas no desempenho poderiam ser alcançadas otimizando a implementação do mecanismo de avaliação de expressões e mapeamento de parâmetros.
