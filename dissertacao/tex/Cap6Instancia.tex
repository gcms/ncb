...
Uma vez construído um modelo que representa a camada NCB, sua execução junto ao ambiente fornecido foi avaliada em comparação à implementação existente da NCB, que integra a CVM. Para realizar essa avaliação foi elaborado um conjunto de cenários de comunicação, que descrevem uma sequência de interações válidas advindas da camada UCM ou dos \emph{frameworks} de comunicação. A partir da execução desses cenários em ambas implementações da NCB foi possível avaliar o modelo construído em relação à implementação já existente.

Para conduzir essa avaliação, configuramos a implementação existente da NCB com um conjunto de recursos que simulam o comportamento esperado de um \emph{framework} de comunicação, e executamos os cenários elaborados. A partir dessa execução foi possível obter as interações geradas pela NCB em cada um dos cenários propostos. Essa informações foram então utilizadas na construção de testes automatizados, capazes de simular os cenários elaborados em uma camada de intermediação de serviços, e verificar se o seu comportamento foi o mesmo obtido na execução com a camada NCB existente.
